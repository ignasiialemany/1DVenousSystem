"""
This file contains all the basic implementations of functions for arterial model.
"""
...
def p(q):
    alpha = q[0]/A0
    p = p_d + K*(alpha**m-alpha**n)
    return p

def c(q):
    alpha = q[0]/A0
    c = np.sqrt(K/rho * (m*alpha**m - n*alpha**n))
    return c
